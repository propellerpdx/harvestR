  ...
  query=NULL)
{
  ...
  response_df <- url %>%
    purrr::map(., function(x) httr::GET(x,httr::add_headers("Harvest-Account-ID" = user,Authorization = key,'User-Agent=Propeller R API Helper (mdruffel@propellerpdx.com)', 'From' = email), query = query)) %>%
